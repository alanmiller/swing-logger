...
class Database:
    ...
    def insert_swing(self, swing_data):
        with self.conn:
            self.conn.execute('''INSERT INTO swings (timestamp,  club, speed, spin_axis, total_spin, hla, vla, club_speed, back_spin, side_spin, path, face_to_target, angle_of_attack, speed_at_impact)
                                 VALUES (:timestamp, :club, :speed, :spin_axis, :total_spin, :hla, :vla, :club_speed, :back_spin, :side_spin, :path, :face_to_target, :angle_of_attack, :speed_at_impact)''', swing_data)
            self.conn.commit()
